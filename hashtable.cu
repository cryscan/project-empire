...
void lookup_hashtable(KeyValue* pHashTable, KeyValue* kvs, uint32_t num_kvs)
{
    ...
    // lookup all the values in hash table
    int gridsize = ((uint32_t)num_kvs + threadblocksize - 1) / threadblocksize;
...
__global__ void gpu_lookup_compare_hashtable(KeyValue* hashtable, KeyValue* kvs, uint32_t num_kvs)
{
    ...
    if (threadid < num_kvs)
    {
        ...
        while (true)
        {
            // found key & compare: delete from kvs
            if (hashtable[slot].key == key && hashtable[slot].value < kvs[threadid].value) 
            {
                kvs[threadid].value = kEmpty;
                break;
            }
            // key not in hashtable: do nothing
            if (hashtable[slot].key == kEmpty)  
            {
                return;
            }
    ...
    __syncthreads();
    // update kvs and num_kvs after deletion
    if (threadid < num_kvs)
    {
        uint32_t key = kvs[threadid].key;
        uint32_t slot = hash(key);
...
// assumption: all keys can be found in hashtable (in global memory)
// lookup the key value in kvs (in shared memory) from hashtable (in global memory)
// compare the value in kvs with the value (if found) in hashtable
// if hashtable value is smaller, remove key-value pair from kvs
// after the operation, the kvs and num_kvs is updated for the deletion

void lookup_compare_delete_hashtable(KeyValue* hashtable, KeyValue* kvs, uint32_t num_kvs)
{

}

// for the rest of the kvs key-value pairs, update the value in hashtable (in global memory)
// return the key-value pairs which need to be pushed to PQs.